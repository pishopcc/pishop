...
// +---------------------------------------------------------------------
// | Author: zhibinm <113664000@qq.com> Date: 2018-04-22 20:21:54
// +---------------------------------------------------------------------
namespace pishop\lib;
/**
 * 权限认证类
 * 功能特性：
 ...
 * 3，一个用户可以属于多个用户组(think_auth_role_user表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(think_auth_role 定义了用户组权限)
 * 
 * 4，支持规则表达式。
...
class Auth{
    ...
    protected $_config = array(
        ...
        'auth_role_user'   => 'auth_role_user', // 用户-用户组关系表
    ...
    public function __construct() {
        ...
        if (Config::get('auth_cofig')) {
            //可设置配置项 AUTH_CONFIG, 此配置项为数组。
            $this->_config = array_merge($this->_config, C('auth_cofig'));
        }
    }
    ...
    public function getGroups($uid) {
        ...
        if (isset($groups[$uid]))
            return $groups[$uid];
        $user_groups = Db::table($this->_config['auth_role_user'] . ' t1')
            ->join($this->_config['auth_role']." t2","t1.roid=t2.roid")
            ->where("t1.uid='$uid' and t2.status='1'")
            ->field('uid,t2.roid,title,rules')->select();
