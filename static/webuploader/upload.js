	...
	function webuploader(opts){
		...
		uploader.on( 'uploadSuccess', function( file,data) {
			...
			if(data.state == 'SUCCESS'){
				...
				sLi += '<img src="' + data.url + '" onerror="this.src=\'__ROOT__/public/plugins/uploadify/nopic.png\'">';
	...
	window.Manager = {
		...
		init : function(){
			...
			$('ul.choose-btns > li.sure').click(function(){
				// alert($("input[name^='fileurl_tmp']").length);
				if($("input[name^='fileurl_tmp']").length>=chooseNum){
					layer.msg('最多选择'+chooseNum+"个",function(){});
					return;
				}
			
				var $checkedFile = $(this).parents('.layui-tab-item').find('div.file-list li.checked');
		...
		//选中文件
		sure : function($file){
			var data = $file.data('img');
			var sLi = "";
			sLi += '<li class="img">';
			sLi += '<img src="' + data + '" onerror="this.src=\'__ROOT__/public/plugins/uploadify/nopic.png\'">';
